        ...
        while (fgets(buffer, sizeof(buffer), stdin)) {
                ...
                if (buffer[0] == ' ' || buffer[0] == '\t') {
                        ...
                        while (isspace(name_untrimmed[first_index])
                               && first_index < sizeof(name_untrimmed))
                                first_index++;

                        /* now trim trailing whitespace: */
                        unsigned last_index = strlen(name_untrimmed);
                        while ((isspace(name_untrimmed[last_index])
                           || name_untrimmed[last_index] == '\0')
                               && last_index > 0)
                                last_index--;

                        /* check whether the space after fprint is all
                         * white space */
                        if (first_index <= last_index) {
                                /* replace name with trimmed version & 
                                 * terminate */
                                name_length = last_index - first_index + 1;
                                char *name = malloc(512);
                                memcpy(name, &name_untrimmed[first_index], 
                                       name_length);
                                name[name_length] = '\0';

                                /* the fingerprint must be converted into an 
                                 * atom now because we need to compare them to 
                                 * each other */
                                const char *fingerprint_atom = 
                                                      Atom_string(fingerprint);

                                /* retrieve the existing list of names from the
                                 * table corresponding to the fingerprint we're
                                 * looking at for this line. (there might not 
                                 * be one yet.) */
                                List_T name_list = Table_get(table,
                                                             fingerprint_atom);

                                /* push the name to the top of the list we 
                                 * received and overwrite the old list */
                                name_list = List_push(name_list, name);

                                /* put the modified list back into the table 
...
void print_group(const void *key, void **value, void *cl)
{
        ...
        /* if there's only one name to this fingerprint, don't print 
         * anything */
        if (List_length(*value) > 1) {
                List_map(*value, print_name, NULL);
                printf("\n"); /* print an extra newline at the end of the 
                               * group */
        }
